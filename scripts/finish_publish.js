// finish_publish.js - 收尾：关闭封面选择弹窗 -> 点编辑页发布 -> 处理确认（最终修正版 v3, 2026-08-21）
// 自包含：仅依赖 ./cdp_lib.js + node_modules/ws，不再依赖 xbrowser。所有点击用 CDP 真实鼠标坐标。
...
function scr(sock, n) { return cdpLib.cdpShot(sock, SAVE + 'fp_' + n + '.png').catch(function () {}); }
...
function isModalOpen(sock) { return cdpLib.cdpEval(sock, "(" + COVER_MODAL + ")?'true':'false'"); }
...
async function main() {
  ...
  // 1) 关封面弹窗
  if (await isModalOpen(sock) === 'true') {
    log('封面弹窗开着，尝试关闭...');
    ...
    if (await isModalOpen(sock) === 'true') {
      for (let attempt = 0; attempt < 4; attempt++) {
        ...
        await sl(2000);
        if (await isModalOpen(sock) === 'false') break;
      }
    }
    for (let i = 0; i < 10; i++) {
      await sl(1000);
      if (await isModalOpen(sock) === 'false') { log('封面弹窗已关闭'); break; }
      if (i === 9) log('封面弹窗仍未关闭');
    }
  } else {
    log('封面弹窗未开（可能已关）');
